...
module.exports = {
	...
	searchHotel: function(add, callback){
		var sql = "select * from hotels where hotelAddress='"+add+"'";
		db.getResult(sql, [], function(results){
			callback(results);
		});
	},
	...
	insert: function(user, callback){
		...
		db.execute(sql2,['',user.username,user.address,user.email,user.contact,user.cpass,"hr"], function(status){
